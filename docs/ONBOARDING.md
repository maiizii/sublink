...
2. **跳转与统计**
   ...
   - 站点设置支持维护多域名列表（`managed_domains`），平台会统一归一化协议/大小写并同步 `www.` 映射；创建短链时需指定列表内的域名，普通账号无法越权。
   - 自 v1.10.8.2 起新增 `subdomain_blacklist` 保留前缀表，启动时会注入 `www`、`mail`、`api`、`admin` 等默认限制，管理员可在后台设置页或 `/api/subdomain-blacklist` 接口批量维护，普通账号在提交子域时会触发长度与名单校验。
3. **认证与安全**
