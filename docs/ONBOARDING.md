...
自 **v1.10.9.2** 起，管理后台与文档持续维持双语支持，并新增根域跳转至后台与循环式一键安装菜单，方便全球团队协同维护。
...
2. **跳转与统计**
   ...
   - 站点设置支持维护多域名列表（`managed_domains`），平台会统一归一化协议/大小写并同步 `www.` 映射；首个域名自动成为平台主域，用于后台默认选项与所有对外展示的域名文案，其余域名可用作短链跳转但不会改变品牌域名文字，普通账号创建短链时必须在列表内选择。
   - 管理后台的语言切换控件位于右上角，当前支持简体中文与英文；系统会记住最近的选择，方便跨地区成员协同操作。
   - 自 v1.10.8.3 起文档同步强调上述主域职责，并继续提供 `subdomain_blacklist` 保留前缀表：启动时会注入 `www`、`mail`、`api`、`admin` 等默认限制，管理员可在后台设置页或 `/api/subdomain-blacklist` 接口批量维护，普通账号在提交子域时会触发长度与名单校验。
3. **认证与安全**
   - 当前版本使用 HTTP Basic + 服务端会话，支持在后台创建多位用户并分配管理员权限；建议配合 Cloudflare/防火墙限制来源 IP，并在 `.env` 中配置强密码与 `SESSION_SECRET`。
   - 参考根目录 `README.md` 与 `docs/` 相关文档梳理日志留存与备份策略；新用户需登录后及时通过「修改密码」入口轮换凭据。
4. **可观测性与运维**
   - 通过 `docker compose logs`、`scripts/proxy-smoke.sh` 等工具快速定位入口问题。
   - 结合 `docs/backup-example.sh` 制定数据库与配置的定期备份方案。

## 上手建议
1. **启动生产环境**：按照根目录 `README.md` 的「一键部署」或「手动部署」流程完成 `.env`、证书与 `docker compose up -d --build`，确认 `https://<域名>/admin` 可登录。
